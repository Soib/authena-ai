...
const LEGITIMACY_THRESHOLD = 0.40
 ...
 */
export const calculateIdScrappingResultScore = (idScrappingResult: DocumentResult): number => {
...
export const calculateLegitimacy = (idScrappingResult: DocumentResult, faceComperingResultScore: number) => {
  const idScrappingResultScore = calculateIdScrappingResultScore(idScrappingResult);

  const reversedIdScrappingResultScore = 1 - faceComperingResultScore;

  const legitimacyScore = (idScrappingResultScore + reversedIdScrappingResultScore) / 2;
